...
if(WIN32)
    ...
    if(NASM_EXECUTABLE)
        message(STATUS "NASM found: ${NASM_EXECUTABLE} — OpenSSL will use assembly optimisations")
        set(OPENSSL_CONFIGURE_COMMAND ${_OPENSSL_WIN_BASE})
    else()
        message(STATUS "NASM not found — OpenSSL will be built without assembly optimisations (no-asm)")
        set(OPENSSL_CONFIGURE_COMMAND ${_OPENSSL_WIN_BASE} no-asm)
    endif()
...
# ============================================================================
# FindOpenSSL.cmake shim
# Intercepts find_package(OpenSSL) calls from downstream dependencies
# (e.g., jwt-cpp) so they resolve via our pre-built imported targets
# instead of triggering a system search that fails on Windows fresh builds.
# Compatible with CMake 3.20+ (file(CONFIGURE) requires 3.18+).
# ============================================================================
set(_OPENSSL_SHIM_DIR "${CMAKE_BINARY_DIR}/_cmake_openssl_shim")
file(MAKE_DIRECTORY "${_OPENSSL_SHIM_DIR}")

file(CONFIGURE
    OUTPUT "${_OPENSSL_SHIM_DIR}/FindOpenSSL.cmake"
    CONTENT [=[
# FindOpenSSL.cmake shim — generated by BuildOpenSSL.cmake.
# Uses OpenSSL::SSL / OpenSSL::Crypto imported targets when available.
cmake_minimum_required(VERSION 3.20)
...
if(TARGET OpenSSL::SSL AND TARGET OpenSSL::Crypto)
    ...
    get_target_property(_ssl_includes OpenSSL::SSL INTERFACE_INCLUDE_DIRECTORIES)
    if(_ssl_includes)
        list(GET _ssl_includes 0 OPENSSL_INCLUDE_DIR)
    endif()
...
# Imported targets not yet created — BuildOpenSSL.cmake must be included first.
if(OpenSSL_FIND_REQUIRED)
    message(FATAL_ERROR
        "OpenSSL::SSL and OpenSSL::Crypto imported targets were not found. "
        "Ensure BuildOpenSSL.cmake is included before any subdirectory that "
        "calls find_package(OpenSSL).")
endif()
...
# Prepend so this shim takes precedence over CMake's built-in FindOpenSSL.cmake
list(PREPEND CMAKE_MODULE_PATH "${_OPENSSL_SHIM_DIR}")
